...
- **`api.freya-hub.fr` reste public mais ne sert plus que cinq routes** (toutes les autres répondent 404) : `POST /invoices/create` et `POST /invoices/credit/create` (webhooks Shopify, signature `X-Shopify-Hmac-Sha256` vérifiée par l'API avec `SHOPIFY_WEBHOOK_SECRET`, requête refusée si `SHOPIFY_WEBHOOK_ENFORCE=true`), `GET /shopify/products` réservé à l'IP du serveur Freya Pro (`15.188.66.87`, lecture du stock entrepôt), et `GET /droppex/order` et `GET /bordereaux-temp/*` (PDF du bordereau renvoyé en lien de téléchargement) (extension Shopify « Droppex bulk print » de `shopify-apps/droppex-slip-print`, appelée depuis le navigateur dans l'admin Shopify ; rouverte le 2026-09-25, **encore sans authentification** — à sécuriser par jeton de session Shopify). Ajouter une route publique = une `location` explicite dans ce vhost, et vérifier avant toute restriction qu'aucun client (extensions Shopify comprises) n'appelle une route fermée.
