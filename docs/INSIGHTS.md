...
- **`recordedAt` à minuit UTC** (jamais une heure de poll réaliste) - marqueur volontaire pour rester distinguable d'un vrai poll a posteriori, sans colonne dédiée.
- Script ponctuel (`npm run backfill:inventory-from-mongo -- --execute`, nécessite `MONGO_STOCK_URL` en variable d'environnement au moment du run, jamais commité) - un seul run nécessaire, l'historique ne change pas rétroactivement.
